...
export default function PrivacyPage() {
  ...
  return (
    <Box minH="100vh" w="100vw" position="relative" overflow="hidden" bg="yellow.50">
      <GrainGradient 
        style={{ height: '100vh', width: '100%' }}
        colorBack="hsl(43, 74%, 95%)"
        softness={0.7}
        intensity={0.7}
        noise={0.6}
        shape="corners"
          ...
          <div className="max-w-2xl mx-auto space-y-4">
            <AnimatePresence>
              {(showSuccess || showError) && (
                <motion.div 
                  ...
                  className={`text-sm rounded-md p-3 border border-yellow-200 ${showSuccess ? 'bg-yellow-50 text-amber-800' : 'bg-red-50 text-red-800'}`}
                >
            ...
            <form className="hs-form" onSubmit={handleSubmit}>
              <div className="flex flex-col items-center gap-2 sm:flex-row border border-gray-200 rounded-lg p-1">
                ...
                <button 
                  type="submit"
                  className="w-full sm:w-auto whitespace-nowrap py-3 px-6 inline-flex justify-center items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-gradient-to-r from-amber-500 to-yellow-600 text-white hover:from-amber-600 hover:to-yellow-700 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:ring-offset-2 transition-all shadow-md hover:shadow-lg"
